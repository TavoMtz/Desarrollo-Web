            ...
            foreach ($categories as $category) {
                ...
                if (!empty($products)) {
                    foreach ($products as $product) {
                        ...
                        // MODIFICACIÓN AQUÍ: Ruta de la imagen con '../'
                        ...
                        // MODIFICACIÓN AQUÍ: Data-image con '../'
...
cart.forEach((item, index) => {
    ...
    const imageUrl = item.image; // Asegúrate de que item.image tiene la ruta '../resources/img/productos/...'

    cartItem.innerHTML = `
        <img src="${imageUrl}" alt="${item.name}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px;">
        <p>${item.name}</p>
...
fetch('update_cart.php', {
    ...
    body: JSON.stringify(cart), // Send the updated cart (even if empty)
        ...
        function confirmPurchase() {
            ...
            if (confirmation) {
                fetch('procesar_pedido_simulado.php', { // Asegúrate de que la ruta sea correcta
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                ...
                .then(data => {
                    if (data.status === 'success') {
                        alert("El pedido se realizará una vez confirmado el Pago. No. de Orden: " + data.no_orden + ", Precio Total: $" + data.precio_total.toFixed(2) + ", Cantidad Total: " + data.cantidad_total);
                        ...
                        alert("Error al realizar el pedido: " + (data.message || 'Inténtalo de nuevo.'));
                    }
                }).catch(error => {
